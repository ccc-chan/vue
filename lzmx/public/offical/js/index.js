$(function () {
    ...
    $.ajax({
        type: "get",
        url: 'http://api.7171.la/api/banner?token=xmsy&code=fe0867b90b2a28b3df69eb8d7bd96681',
        dataType: 'json',
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i =0;i<data.length;i++){
                ...
                }else if(data[i].type == '155'){
                    if(coList.length < 4 ){
                        $(".co").append(co);
                        $(".co >div").eq(coList.length).find('a').attr({'href':"/page2?id="+data[i].id,"target":"_blank"});
