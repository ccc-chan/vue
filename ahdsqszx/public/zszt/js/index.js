$(function () {
    ...
    $.ajax({
        type: "get",
        url: 'http://api.7171.la/api/article?token=dsqs&code=7eeb23e92ad80d4b62a73d92fd2fa2b7',
        dataType: 'json',
            ...
            for (var i =0;i<data.length;i++){
                if(data[i].type == '199'){
                    if(enrollmentList.length < 5 ){
                        ...
                        $(".enrollmentList .list-item").eq(enrollmentList.length).find('a').attr('href',"/page1?id="+data[i].id);
                ...
                }else if(data[i].type == '200'){
                    if(polickList.length < 5 ){
                        $(".polickList").append(templet);
                        $(".polickList .list-item").eq(polickList.length).find('a').attr('href',"/page2?id="+data[i].id);
