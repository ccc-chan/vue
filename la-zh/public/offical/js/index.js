$(function () {
    document.title="六安振华职业中专学校特色专业网";
    var dyn = '<div><a>\n' +
   ...
   var templet = '<div class="list-item"><a><i class="iconfont icon-dayin_zhizhangshezhi"></i><p></p></a></div>';
    $.ajax({
        type: "get",
        url: 'http://api.7171.la/api/article?token=lhzh&code=06ed5113da04cd410428f12e74c4b666',
        dataType: 'json',
        success: function (data) {
            ...
            for (var i =0;i<data.length;i++){
                if(data[i].type == '98'){
                    if(dynList.length < 4 ){
                        $(".dyn-list").append(dyn);
                        $(".dyn-list >div").eq(dynList.length).find('a').attr('href',"/page1?id="+data[i].id);
