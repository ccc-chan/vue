$(function () {
    ...
    $.ajax({
        type: "get",
        url: 'http://api.7171.la/api/article?token=hszd&code=71777b875d8acc590e21885dcea9f676',
        dataType: 'json',
        success: function (data) {
            ...
            for (var i =0;i<data.length;i++){
                ...
                }else if(data[i].type == '151'){
                    if(polickList.length < 5 ){
                        $(".polickList").append(templet);
                        $(".polickList .list-item").eq(polickList.length).find('a').attr({'href':"/zszt/page1?id="+data[i].id,"target":"_blank"});
