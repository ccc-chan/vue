                ...
                function jump(n) {
                    ...
                    for (var i = 0; i < show; i++) {
                        if ((n-1) * show + i >= data.length) {
                            ...
                            $(".action-news-list>div").eq(i).find('a').attr('href',"/news_page"+n+"?id="+data[(n-1) * show + i].id);
                    ...
                    if(curPage == 0){
                        ...
                        for (var i = 0; i < show; i++) {
                            if ((curPage - 1) * show + i < 0) {
                                ...
                                $(".action-news-list").append(templet);
                                $(".action-news-list>div").eq(i).find('a').attr('href',"/news_page"+n+"?id="+data[(curPage - 1) * show + i].id);
                                $(".action-news-list>div").eq(i).find('a').attr('target','_blank');
                    ...
                    if (curPage > totalPage+1)
                        ...
                        for (var i = 0; i < show; i++) {
                            if (ind * show + i >= data.length) {
                                ...
                                $(".action-news-list").append(templet);
                                $(".action-news-list>div").eq(i).find('a').attr('href',"/news_page"+n+"?id="+data[ind * show + i].id);
                                $(".action-news-list>div").eq(i).find('a').attr('target','_blank');
