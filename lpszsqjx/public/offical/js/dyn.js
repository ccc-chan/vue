$(function () {
    document.title="学校动态_六盘水钟山区职业技术学校特色专业网";
    $.ajax({
        type: "get",
        url: 'http://api.7171.la/api/article?token=lpszs&code=f3084700e06bee7bf61ed40ddbb376b5&type=114',
        dataType: 'json',
        success: function (data) {
            console.log(data)
            var templet = '<div><a>\n' +
                '  <div class="col-lg-2 col-md-3 col-sm-3 col-xs-4 imgThumb"><img class="theme-img"/></div>\n' +
            ...
            function jump(n) {
                ...
                for (var i = 0; i < show; i++) {
                    if ((n-1) * show + i >= data.length) {
                        ...
                        $(".theme-list>div").eq(i).find('a').attr('href',"/news_page1?id="+data[(n-1) * show + i].id);
                ...
                if(curPage == 0){
                    ...
                    for (var i = 0; i < show; i++) {
                        if ((curPage - 1) * show + i < 0) {
                            ...
                            $(".theme-list").append(templet);
                            $(".theme-list>div").eq(i).find('a').attr('href',"/news_page1?id="+data[(curPage - 1) * show + i].id);
                            $(".theme-list>div").eq(i).find('a').attr('target','_blank');
                ...
                if (curPage > totalPage+1)
                    ...
                    for (var i = 0; i < show; i++) {
                        if (ind * show + i >= data.length) {
                            ...
                            $(".theme-list").append(templet);
                            $(".theme-list>div").eq(i).find('a').attr('href',"/news_page1?id="+data[ind * show + i].id);
                            $(".theme-list>div").eq(i).find('a').attr('target','_blank');
